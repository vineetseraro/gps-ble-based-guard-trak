...
module.exports.postAuthenticateUser = (event, context, callback) => {
  ...
  commonHelper
    ...
    .then(dbURI => {
      ...
      if (userAttributes && userAttributes['custom:isAdminApproved'] === 'yes' && userAttributes['sub']) {
        userHelper
        ...
        // // check if things are linked to user to check if user is already logged in
        // userHelper
        // .getById(userAttributes['sub'])
        // .then((result) => {
        //   if ( result.things && result.things.length ) {
        //     const error = new Error('User is already logged in on some devices. Please logout from all other devices');
        //     mongoose.disconnect();
