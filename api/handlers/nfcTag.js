...
module.exports.getNfcTags = (event, context, callback) => {
  commonHelper.decryptDbURI().then(dbURI => {
    ...
    bluebirdPromise
      ...
      .then(resultObj => {
        const response = akResponse.listSuccess(resultObj, messages.TAG_LIST);
        mongoose.disconnect();
...
module.exports.getNfcTagbyId = (event, context, callback) => {
  commonHelper.decryptDbURI().then(dbURI => {
    ...
    nfcTagHelper
      ...
      .then(result => {
        const response = akResponse.success(result, messages.TAG_FETCH_SUCCESS, messages.OK);
    ...
    nfcTagHelper
      ...
      .then(populatedEvent => {
        nfcTagHelper
          ...
          .then(result => {
            const response = akResponse.created(result, messages.TAG_SAVE_SUCCESS, messages.OK);
