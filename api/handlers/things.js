 ...
 * @callback callback Lambda Callback
 */
module.exports.beaconLocation = (event, context, callback) => {
  commonHelper.decryptDbURI().then(dbURI => {
    event = commonHelper.parseLambdaEvent(event);
    clientHandler.setClient(clientHandler.getClientObject(event));
    currentUserHandler.setCurrentUser(currentUserHandler.getCurrentUserObject(event));
    ...
    commonHelper.connectToDb(dbURI);
    const thingHelper = require('../helpers/things');
    //return thingHelper.beaconLocation(thingHelper.getFilterParams(event), thingHelper.getExtraParams(event))
    bluebirdPromise
      .all([
        thingHelper.beaconLocation(thingHelper.getFilterParams(event), thingHelper.getExtraParams(event)),
        thingHelper.beaconLocationCount(thingHelper.getFilterParams(event))
      ])
      .then(resultObj => {
        const response = {
          statusCode: 200,
