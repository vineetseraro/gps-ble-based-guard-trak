  ...
  cond.$and = [
    filters,
    {
      attributes: {
        $not: {
          $elemMatch: {
            name: 'major', value: process.env.defaultBeaconMajor
          }
        }
      }
    }
 ...
 * 
 */
thingService.prototype.beaconLocation = function() {
  let filterparams = {'type':'beacon', 'location.id' : { '$exists': true}};
  return thingModel
    .find(filterparams)
    .then(result => {
      const list = [];
      if (result) {
        for (const i in result) {
          if (result.hasOwnProperty(i)) {
            // let i = result[j];
            const nresult = commonHelper.moveSystemAttributesToGlobal(result[i]);
            list.push({
              "id" : result[i]._id,
              "code" : result[i].code,
              "name" : result[i].name,
              "uuid" : nresult.uuid,
              "major" : Number(nresult.major),
              "minor" : Number(nresult.minor),
              "zone" : result[i].location
            });
          }
        }
      }
 ...
 * 
 */
thingService.prototype.beaconLocationCount = function() {
  let filterparams = {'location.id' : { '$exists': true}};
  return thingModel.count(filterparams);
};
