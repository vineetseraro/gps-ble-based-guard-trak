...
const tourActionsModel = require('../models/tourActions');
...
reportService.prototype.get = function(event) {
  ...
  switch (event.queryStringParameters.type) {
    ...
    case 'tour_status':
      return this.tourStatusWidget(event);
    default:
      return [];
  }
};
...
reportService.prototype.formatReportData = function(recordData, reportType) {
  ...
  switch (reportType) {
    ...
    case 'getTours':
      tmp.id = record._id;
      tmp.tourId = record.tourId;
      tmp.tourName = record.tourName;
      tmp.tourStatus = record.tourStatus;
      tmp.attendee = record.attendee;
      tmp.attendee.id = '';
      if(typeof record.userData !== 'undefined') {
          tmp.attendee.id = record.userData._id;
      }
      tmp.attendee.fullName = record.attendee_fullName;
      tmp.task = record.task;
      tmp.from = record.from;
      tmp.to = record.to;
      tmp.device = record.device;
      tmp.duration = record.duration;
      tmp.actions = record.actions;
      tmp.task.from = record.taskData.from;
      tmp.task.to = record.taskData.to;
      tmp.updatedOn = record.updatedOn;
      break;
    case 'getTourEvents':
      tmp.id = record._id;
      tmp.task = record.task;
      tmp.tour = record.tour;
      tmp.attendee = record.attendee;
      tmp.device = record.device;
      //tmp.attendee.id = record.userData._id;
      tmp.attendee.fullName = record.attendee_fullName;
      tmp.tour.name = record.tourName;
      tmp.action = record.action;
      tmp.location = record.location;
      if(record.additionalInfo === null) {
        record.additionalInfo = {};
      }
      tmp.additionalInfo = record.additionalInfo;
      tmp.updatedOn = record.updatedOn;
      tmp.updatedBy = record.updatedBy;
      tmp.updatedBy.fullName = record.updatedBy_fullName;
      break;
    case 'getDeviceTourStats':
      tmp.id = record._id;
      tmp.from = record.from;
      tmp.totalTours = record.totalTours;
      tmp.totalFailedTours = record.totalFailedTours;
      tmp.totalSuccessTours = record.totalSuccessTours;
      tmp.totalInProcessTours = record.totalInProcessTours;
      tmp.device = record.device;
      break;
    case 'getGuardTourStats':
      tmp.id = record._id;
      tmp.from = record.from;
      tmp.totalTours = record.totalTours;
      tmp.totalFailedTours = record.totalFailedTours;
      ...
      tmp.totalInProcessTours = record.totalInProcessTours;
      tmp.attendee = record.attendee;
      if(record.attendee_fullName && record.attendee_fullName.length > 0) {
        tmp.attendee.fullName = record.attendee_fullName[0];
      }
  ...
  switch (reportType) {
    ...
    case 'getTourEvents':
      map = {
        ...
        zone: 'location.floor.zone.name',
        address: 'location.address',
        actionDate: 'action.actionDate',
        updatedOn: 'updatedOn',
        updatedBy: 'updatedBy_fullName'
      };
      break;
    case 'getDeviceTourStats':
      map = {
        ...
        totalFailedTours: 'totalFailedTours',
        totalSuccessTours: 'totalSuccessTours',
        totalInProcessTours: 'totalInProcessTours',       
        from: 'from'
      };
      break;
    case 'getGuardTourStats':
 ...
 * 
 */
reportService.prototype.getTours = function(event) {
  const params = {};
  ...
  params.project = {
    ...
    'taskData.from': 1,
    'taskData.to': 1,
    'attendee_fullName' : {$concat: ['$attendee.firstName', ' ', '$attendee.lastName']},
    'tourName' : {$concat: ['$task.name', ' - ', {$substr:['$tourId', 0, -1 ] }]},
    task: 1,
    from: 1,
    to: 1,
    device: 1,
  ...
  const preferredRole = (currentUserHandler.getCurrentUser() || {}).preferredRole;
  
  const attendeeUuid = (currentUserHandler.getCurrentUser() || {}).uuid;
  /*if(preferredRole !== 'role-guardtrak-akadmin') {
    params.match['attendee.uuid'] = attendeeUuid;
  }*/
  ...
  if (event.queryStringParameters && event.queryStringParameters.filter) {
    ...
    params.match.$or = [
      {
        'tourName': new RegExp(event.queryStringParameters.filter, 'i')
      },
      {
        'attendee_fullName': new RegExp(event.queryStringParameters.filter, 'i')
      },
      {
        'attendee.firstName': new RegExp(event.queryStringParameters.filter, 'i')
      },
      {
        'attendee.lastName': new RegExp(event.queryStringParameters.filter, 'i')
      },
      {
        'task.code': new RegExp(event.queryStringParameters.filter, 'i')
      },
      {
        'task.name': new RegExp(event.queryStringParameters.filter, 'i')
      },
      {
        'device.name': new RegExp(event.queryStringParameters.filter, 'i')
      }
  ...
  if (event.queryStringParameters.device) {
    params.match['device.name'] = new RegExp( event.queryStringParameters.device, 'i' );
  }

  if (event.queryStringParameters.tour) {
    params.match.tourName = new RegExp(event.queryStringParameters.tour, 'i');
  }
  
  if (event.queryStringParameters.deviceId) {
    params.match['device.code'] = event.queryStringParameters.deviceId;
  }
  
  if (event.queryStringParameters.attendee) {
    params.match['userData._id'] = mongoose.Types.ObjectId(event.queryStringParameters.attendee);
  }

  if ( event.queryStringParameters.tourStatus )  {
    params.match.tourStatus = parseInt(event.queryStringParameters.tourStatus, 10);
  }
  
  if (event.queryStringParameters.task) {
    params.match.$or = [
      {
        'task.code': new RegExp(event.queryStringParameters.task, 'i')
      },
      {
        'task.name': new RegExp(event.queryStringParameters.task, 'i')
      }
    ]
  }
  
  if (event.queryStringParameters.tourStartFrom || event.queryStringParameters.tourStartTo) {
    // params.match.from = {};
  } 
  
  params.match.from = {};
  params.match.from.$lt = new Date();
  // params.match.to.$lt = new Date();

  if (event.queryStringParameters.tourStartFrom) {
    params.match.from.$gte = new Date(event.queryStringParameters.tourStartFrom);
  }

  if (event.queryStringParameters.tourStartTo) {
    params.match.from.$lte = akUtils.formatToDateFilter(
      new Date(event.queryStringParameters.tourStartTo)
    );
  }
  
  if (event.queryStringParameters.tourEndFrom || event.queryStringParameters.tourEndTo) {
    params.match.to = {};
  }

  if (event.queryStringParameters.tourEndFrom) {
    params.match.to.$gte = new Date(event.queryStringParameters.tourEndFrom);
  }

  if (event.queryStringParameters.tourEndTo) {
    params.match.to.$lte = akUtils.formatToDateFilter(
      new Date(event.queryStringParameters.tourEndTo)
    );
  }
  // console.log(JSON.stringify(params.match));
  // params.match = clientHandler.addClientFilterToConditions(params.match);
  ...
  const sorting = this.extractSortOptions(event, 'getTours');
  ...
  return bluebirdPromise
    .all([
      this.getToursData(params),
      this.getToursCount(params)
    ])
    .then(
      result =>
        result
...
reportService.prototype.getToursData = function(params) {
  ...
  return toursModel
    ...
    .then(result => {
      let list = [];
      if (result) {
        list = result.map(x => this.formatReportData(x, 'getTours'));
      }
...
reportService.prototype.getToursCount = function(params) {
  return toursModel
    .aggregate([
      {
        $lookup: {
 ...
 * 
 */
reportService.prototype.getTourEvents = function(event) {
  const params = {};
  ...
  params.project = {
    ...
    'userData': 1,
    'attendee_fullName' : {$concat: ['$attendee.firstName', ' ', '$attendee.lastName']},
    'tourName' : {$concat: ['$task.name', ' - ', {$substr:['$tour.tourId', 0, -1 ] }]},
    location: 1,
    action: 1,
    device: 1,
    updatedOn: 1,
    additionalInfo: 1,
    updatedBy: 1,
    'updatedBy_fullName' : {$concat: ['$updatedBy.firstName', ' ', '$updatedBy.lastName']}
  };

  params.match = {};
  //params.match = {
  //  'tour.id': mongoose.Types.ObjectId(event.pathParameters.id)
  //};
  ...
  if (event.queryStringParameters && event.queryStringParameters.filter) {
    ...
    params.match.$or = [
      ...
      {
        'action.actionType': new RegExp(event.queryStringParameters.filter, 'i')
      },
      {
      ...
      {
        'location.address': new RegExp(event.queryStringParameters.filter, 'i')
      },
      {
        'location.floor.zone.code': new RegExp(
          event.queryStringParameters.filter,
          'i'
        )
      },
      {
        'location.floor.zone.name': new RegExp(
          event.queryStringParameters.filter,
          'i'
        )
      },
      {
        'location.floor.code': new RegExp(
          event.queryStringParameters.filter,
          'i'
        )
      },
      {
        'location.floor.name': new RegExp(
          event.queryStringParameters.filter,
          'i'
        )
      }
  ...
  // params.match['tourData.from'] = {};
  // params.match['tourData.from'].$lt = new Date();
  
  if (mongoose.Types.ObjectId.isValid(event.queryStringParameters.tourId)) {
    params.match['tour.id'] = mongoose.Types.ObjectId(
      event.queryStringParameters.tourId
    );
  }
  ...
  if (event.queryStringParameters.address) {
    params.match['location.address'] = new RegExp( event.queryStringParameters.address, 'i' );;
  }
  
  if (mongoose.Types.ObjectId.isValid(event.queryStringParameters.location)) {
    params.match['location.id'] = mongoose.Types.ObjectId(
      event.queryStringParameters.location
    );
  }

  if (mongoose.Types.ObjectId.isValid(event.queryStringParameters.floor)) {
    params.match['location.floor.id'] = mongoose.Types.ObjectId(
      event.queryStringParameters.floor
    );
  }

  if (mongoose.Types.ObjectId.isValid(event.queryStringParameters.zone)) {
    params.match['location.floor.zone.id'] = mongoose.Types.ObjectId(
      event.queryStringParameters.zone
    );
  }

  if (event.queryStringParameters.actionType) {
    params.match['action.actionType'] = event.queryStringParameters.actionType;
  }
  
  if (event.queryStringParameters.attendee) {
    // params.match['attendee.id'] = mongoose.Types.ObjectId(event.queryStringParameters.user);
    params.match['userData._id'] = mongoose.Types.ObjectId(event.queryStringParameters.attendee);
  }
  ...
  if (event.queryStringParameters.actionDateFrom || event.queryStringParameters.actionDateTo) {
    params.match.action.actionDate = {};
  }
  
  params.match['action.actionDate'] = {};
  params.match['action.actionDate'].$lt = new Date();

  if (event.queryStringParameters.actionDateFrom) {
    params.match.action.actionDate.$gte = new Date(event.queryStringParameters.actionDateFrom);
  }

  if (event.queryStringParameters.actionDateTo) {
    params.match.action.actionDate.$lte = akUtils.formatToDateFilter(
      new Date(event.queryStringParameters.actionDateTo)
    );
  }
  ...
  const sorting = this.extractSortOptions(event, 'getTourEvents');
  ...
  return bluebirdPromise
    .all([
      this.getTourEventsData(params),
      this.getTourEventsCount(params)
    ])
    .then(
      result =>
        result
...
reportService.prototype.getTourEventsData = function(params) {
  return tourActionsModel
    .aggregate([
      {
        $lookup: {
          from: 'users',
    ...
    .then(result => {
      let list = [];
      if (result) {
        list = result.map(x => this.formatReportData(x, 'getTourEvents'));
      }
...
reportService.prototype.getTourEventsCount = function(params) {
  // throw new Error('ddd');
  return tourActionsModel
    .aggregate([
      {
...
reportService.prototype.getTourEventDetails = function(event) {
  const params = {};
  ...
  params.match = {};
  params.match._id = mongoose.Types.ObjectId(event.pathParameters.id);
  ...
  return tourActionsModel
    ...
    .then(result => {
      let record = {};
      if (result) {
        record.id = result[0]._id;
        record.task = result[0].task;
        record.tour = result[0].tour;
        record.attendee = result[0].attendee;
        //tmp.attendee.id = record.userData._id;
        record.attendee.fullName = result[0].attendee_fullName;
        record.action = result[0].action;
        record.device = result[0].device;
        record.location = result[0].location;
        record.additionalInfo = result[0].additionalInfo;
        record.updatedOn = result[0].updatedOn;
        record.updatedBy = result[0].updatedBy;
        record.updatedBy.fullName = result[0].updatedBy_fullName;
        // list = result.map(x => this.formatReportData(x, 'getTourEvents'));
      }
 ...
 * 
 */
reportService.prototype.getDeviceTourStats = function(event) {
  const params = {};
  ...
  if(event.queryStringParameters.range) {
    let utcoffset;
    if(event.queryStringParameters.utcoffset) {
      utcoffset = event.queryStringParameters.utcoffset * 60 * 1000;
    } else {
      utcoffset = 0;
    }
    switch(event.queryStringParameters.range) {
      case 'day':
        params.group = { did: '$device.id', 
          month: { $month: { $add: ["$from", utcoffset ]  } }, 
          day: { $dayOfMonth: { $add: ["$from", utcoffset ] } }, 
          year: { $year: { $add: ["$from", utcoffset ] } } };
        break;
      case 'week':
        params.group = { did: '$device.id', 
            week: { $week: { $add: ["$from", utcoffset ] } }, 
            month: { $month:  { $add: ["$from", utcoffset ] } },  
            year: { $year: { $add: ["$from", utcoffset ] } } };
        break;
      case 'month':
  ...
  const sorting = this.extractSortOptions(event, 'getDeviceTourStats');
  ...
  return bluebirdPromise
    .all([
      this.getDeviceTourStatsData(params),
      this.getDeviceTourStatsCount(params)
    ]);
};
//----------------

reportService.prototype.getDeviceTourStatsData = function(params) {
  return toursModel
    .aggregate([
      {
        $match: {
      ...
      },
      {
        $group : {
          ...
          from:{ $first: '$from' },    
          totalTours: {$sum: 1},
          totalFailedTours: {$sum: {
            $cond: [{
              $eq: ["$tourStatus", 30]
            },1,0]
          }},
          totalSuccessTours: {$sum: {
            $cond: [{
              $eq: ["$tourStatus", 20]
            },1,0]
          }},
          totalInProcessTours: {$sum: {
            $cond: [{
              $eq: ["$tourStatus", 10]
            },1,0]
          }}
    ...
    .then(result => {
      let list = [];
      if (result) {
        list = result.map(x => this.formatReportData(x, 'getDeviceTourStats'));
      }
...
reportService.prototype.getDeviceTourStatsCount = function(params) {
  // throw new Error('ddd');
  return toursModel
    .aggregate([
      {
 ...
 * 
 */
reportService.prototype.getGuardTourStats = function(event) {
  const params = {};
  ...
  if(event.queryStringParameters.range) {
    ...
    switch(event.queryStringParameters.range) {
      case 'day':
        params.group = { guardId: '$attendee.uuid', 
          month: { $month: { $add: ["$from", utcoffset ]  } }, 
          day: { $dayOfMonth: { $add: ["$from", utcoffset ] } }, 
          year: { $year: { $add: ["$from", utcoffset ] } } };
        break;
  ...
  if (event.queryStringParameters.attendee) {
    params.match['attendee_fullName'] = new RegExp( event.queryStringParameters.attendee, 'i' );
  }
  ...
  const sorting = this.extractSortOptions(event, 'getGuardTourStats');
  ...
  return bluebirdPromise
    .all([
      this.getGuardTourStatsData(params),
      this.getGuardTourStatsCount(params)
    ]);
};
//----------------

reportService.prototype.getGuardTourStatsData = function(params) {
  return toursModel
    .aggregate([
      {
        $match: {
      ...
      },
      {
        $group : {
          ...
          attendee_fullName : { $addToSet: {$concat: ['$attendee.firstName', ' ', '$attendee.lastName']} },
          from:{ $first: '$from' },    
          totalTours: {$sum: 1},
          totalFailedTours: {$sum: {
            $cond: [{
    ...
    .then(result => {
      let list = [];
      if (result) {
        list = result.map(x => this.formatReportData(x, 'getGuardTourStats'));
      }
...
reportService.prototype.getGuardTourStatsCount = function(params) {
  // throw new Error('ddd');
  return toursModel
    .aggregate([
      {
...
reportService.prototype.tourStatusWidget = function(params) {
  console.log('=================');
  return toursModel
    .aggregate([
      {
